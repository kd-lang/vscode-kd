...
export default class KdCompilerProvider implements vscode.CodeActionProvider {
  ...
  public activate(subscriptions: vscode.Disposable[]) {
    ...
    this.buildDiagnostics = vscode.languages.createDiagnosticCollection("kd");
    this.astDiagnostics = vscode.languages.createDiagnosticCollection("kd");
  ...
  maybeDoASTGenErrorCheck(change: vscode.TextDocumentChangeEvent) {
    if (change.document.languageId !== "kd") return;
  ...
  private _doASTGenErrorCheck(change: vscode.TextDocumentChangeEvent) {
    ...
    if (textDocument.languageId !== "kd") {
      return;
    }
    const kd_path = config.get("kdPath") || "kd";
    const cwd = vscode.workspace.getWorkspaceFolder(textDocument.uri).uri
      .fsPath;

    let childProcess = cp.spawn(kd_path as string, ["ast-check"], { cwd });
  ...
  private _doCompile(textDocument: vscode.TextDocument) {
    ...
    let decoded = "";
    let childProcess = cp.spawn("kd", processArg, { cwd });
    if (childProcess.pid) {
      // @ts-ignore
